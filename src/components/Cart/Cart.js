...
export default function Cart(props) {
	...
	// Everything will be stored in cents
	function handleCurrencyCalc(value) {
		const currentSubtotalInCents = parseFloat(subtotal) * 100;
		const addingValueInCents = parseFloat(value) * 100;
		const newSubTotal = (currentSubtotalInCents + addingValueInCents).toString();
    ...
    return (
        <div className="cart bg-white p-3">
            <div className="cart-header mb-2">
                <h4 className="mb-0">
                    Shopping Cart
                    {hasItems && <button className="btn btn-link p-0" onClick={() => setDisplayItems(!displayItems)}>Show items</button>}
                </h4>
            </div>
                    ...
                    <ul>
                        {cart.map((item, i) => (
                            <li className="d-flex align-items-start" key={i}>
                                <button className="btn p-0 mr-2" onClick={() => setCart(cart.filter(cartItem => cartItem.id !== item.id))}
